...
import webview
from utils.dataprocess import format, souped, total_name
...
class City():
    ...
    def __init__(self, name:str, url:str=None):
        ...
        self.hightemp : list[int] = []
        self.lowtemp : list[int] = []
...
class Province():
    ...
    def __init__(self, name:str):
        ...
        self.map_chart.set_global_opts(
                title_opts=opts.TitleOpts(
                    title="%s15天温度预报图" % total_name(self.name),
                    pos_top="2%",
                    pos_left="2%",
    ...
    #return date list
    def datelist(self, index=0):

        return self.cities[index].date
    ...
    def show(self, auto_display=False): 
        ...
        html_name = "%s%s地图.html" % (total_name(self.name), series_name)
        timeline.render(html_name)

        # if auto_display:
        #     webbrowser.open(html_name)

        webview.create_window('15天天气预报可视化查询', html_name, js_api=self, width=1500, height=900)
        webview.start()